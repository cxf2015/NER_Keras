...
def viterbi(obs_len, states_len, start_p, trans_p, emit_p):
    #
    # print('start_p:',start_p,np.shape(start_p))
    # print('trans_p:',trans_p,np.shape(trans_p))

    emit_p=np.reshape(emit_p,(np.shape(emit_p)[0],np.shape(emit_p)[1]))
    # print('emit_p:',np.shape(emit_p))
    # print('emit_p:',emit_p,np.shape(emit_p))

    lenObs = obs_len #len(obs[0])
    lenStates = states_len#len(states)
    # print(lenObs,lenStates)
    ...
    for y in range(lenStates):
        ...
        path[y][0] = y
        # print(0,y,start_p[y] * emit_p[y][0])
    # V[0][88]=0
    #t > 1时
    ...
    # print('V:',V)
    # print('v.argmax:',np.argmax(V,axis=1))
    for y in range(lenStates):
